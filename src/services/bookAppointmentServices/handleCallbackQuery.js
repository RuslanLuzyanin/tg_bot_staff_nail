...
const callbackHandlers = {
    ...
    show_next_dates: (ctx) => ctx.editMessageText('Выберите дату:', getDateMenu(24, false, true)),
    show_prev_dates: (ctx) => ctx.editMessageText('Выберите дату:', getDateMenu(0)),
    ...
    back_to_date_menu: (ctx) => ctx.editMessageText('Выберите дату:', getDateMenu()),
    back_to_time_menu: (ctx) => ctx.editMessageText('Выберите время:', getTimeMenu()),
...
function handleCallbackQuery(ctx) {
    ...
    if (callbackData.startsWith('select_date_')) {
        handleDateSelection(ctx);
        ctx.editMessageText('Выберите время:', getTimeMenu());
        return;
    }
